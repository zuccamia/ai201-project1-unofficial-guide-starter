...
Writing planning.md before any code made this project noticeably more focused than other vibe-coding projects I've worked on. With chunking parameters, the embedding model, top-k, and tier-aware ranking manually verified and already decided on paper, each Claude prompt could be short and pointed (e.g. "implement chunk_text() per ingestion.md"), saving tokens that would otherwise have gone into open-ended deliberation and throwaway implementations.

**One way your implementation diverged from the spec, and why:**
The one place implementation drifted off-spec was Milestone 5's system prompt: I copy-pasted a grounded-answer template from another project as-is, which mistakenly signaled that I preferred its relevance-first ranking over my own tier-first design. Claude flagged the divergence in the docs as it implemented, so I caught the mistake and reverted to tier-anchored language.
